...
function main() {
	...
	if (options.includeBounds) {
		...
		strScadCode += options.name + "_shapeBounds=[" +
			shapes.map(function(shape){
				var bounds = pointsBounds(shape.points);
				return "[" + strPointMm(bounds.min) + ", "+ strPointMm(bounds.max) +"]"
			}).join(", ") + "];\n";
	}

	if (options.includeCenters) {
		strScadCode += options.name + "_overallCenter=" + strPointMm(center) + ";\n";
		strScadCode += options.name + "_shapeCenters=" + strPointsMm(shapes.map(function(shape){ return pointsCenter(shape.points); })) + ";\n";
	}
	...
	if (options.includePoints) {
		...
		if (options.polygonModule) {
			shapes.forEach(function(shape, i) {
				...
				strScadCode += "\tlinear_extrude(height = 30, center = false) {\n";
				if (shape.paths) {
		...
		if (options.includeShapeInvocation) {
...
function showOptionsDialog() {
	...
	includePointsCheckbox.value = includeBoundsCheckbox.value = includeCentersCheckbox.value = polygonModuleCheckbox.value = includeShapeInvocationCheckbox.value = true;
